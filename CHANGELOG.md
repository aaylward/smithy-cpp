...
- Phase 8 groundwork, wire-format-first (ADR-0014): `//runtime:eventstream`
  ...
  against — CRC-guarded prelude, ten typed header wire types, opaque `Blob`
  payloads, an incremental strict fail-closed decoder, and symmetric bounds
  (Encode refuses whatever Decode would reject). Fuzzed, and pinned with
  byte-exact vectors from an independent implementation.
