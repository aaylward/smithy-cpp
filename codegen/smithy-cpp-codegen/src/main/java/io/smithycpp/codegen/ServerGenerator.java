...
final class ServerGenerator {
  ...
  private void writeHeader(CppWriter w) {
    ...
    w.write("/// The context carries what the typed input doesn't model: the raw request");
    w.write("/// (unmodeled headers, the inbound traceparent, the transport-stamped peer");
    w.write("/// address) plus the routing captures; leave the parameter unnamed when an");
    w.write("/// operation has no use for it.");
    ...
    for (OperationShape operation : operations) {
      ...
      w.write(
          "virtual smithy::Outcome<$L> $L(const $L& input,"
