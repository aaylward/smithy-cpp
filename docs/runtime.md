...
| `//runtime:http` | `smithy::http` | `Headers` (case-insensitive), URI percent-encoding per the Smithy HTTP binding rules, `HttpRequest`/`HttpResponse`, `HttpClient`/`HttpServerTransport` interfaces, `Loopback` in-memory transport, built-in `SocketHttpClient`/`SocketHttpServer` (test/reference only — ADR-0006), W3C `TraceContext` parse/format/generate, the transport-neutral `WebSocket` interface (blocking calls plus the ADR-0019 completion-driven `ReceiveAsync`/`SendAsync` twins) + `InMemoryWebSocketPair` (the streaming Loopback analog, async-capable, so generated streaming tests run without Boost — ADR-0016), and the coroutine adapter `AsyncEventStream`/`Detached` (ADR-0019: `co_await` where the blocking facade parks a thread) |
