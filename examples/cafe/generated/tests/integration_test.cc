...
AlternativeMilk RandomAlternativeMilk(Rng& rng) {
  AlternativeMilk v{};
  v.kind = rng.Text(1, 9);
  return v;
}

GetOrderInput RandomGetOrderInput(Rng& rng) {
  GetOrderInput v{};
  v.orderId = rng.Text(1, 9);
  return v;
}

CancelledStatus RandomCancelledStatus(Rng& rng) {
  CancelledStatus v{};
  if (rng.Coin()) v.reason = rng.Text(1, 9);
  return v;
}

PendingStatus RandomPendingStatus(Rng& rng) {
  PendingStatus v{};
  v.position = static_cast<std::int32_t>(rng.Int(-2147483648LL, 2147483647LL));
  return v;
}

ReadyStatus RandomReadyStatus(Rng& rng) {
  ReadyStatus v{};
  v.readyAt = smithy::Timestamp::FromEpochMilliseconds(rng.Int(0, 4102444799LL) * 1000);
  return v;
}

OrderStatus RandomOrderStatus(Rng& rng) {
  switch (rng.engine() % 3) {
    case 0:
...
GetOrderOutput RandomGetOrderOutput(Rng& rng) {
  GetOrderOutput v{};
  v.orderId = rng.Text(1, 9);
...
OrderNotFound RandomOrderNotFound(Rng& rng) {
  OrderNotFound v{};
  v.orderId = rng.Text(1, 9);
  return v;
}

DairyMilk RandomDairyMilk(Rng& rng) {
  DairyMilk v{};
  v.percentFat = static_cast<float>(rng.Int(-8000000LL, 8000000LL)) / static_cast<float>(8);
  return v;
}

MilkOption RandomMilkOption(Rng& rng) {
  switch (rng.engine() % 3) {
    case 0:
...
OrderCoffeeInput RandomOrderCoffeeInput(Rng& rng) {
  OrderCoffeeInput v{};
  v.coffeeType = CoffeeType::FromString(std::array<const char*, 4>{"DRIP", "ESPRESSO", "CORTADO", "LATTE"}[rng.engine() % 4]);
...
OrderCoffeeOutput RandomOrderCoffeeOutput(Rng& rng) {
  OrderCoffeeOutput v{};
  v.orderId = rng.Text(1, 9);
  v.status = RandomOrderStatus(rng);
  return v;
...
OutOfBeans RandomOutOfBeans(Rng& rng) {
  OutOfBeans v{};
