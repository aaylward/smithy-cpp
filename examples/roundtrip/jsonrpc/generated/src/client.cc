...
smithy::Outcome<RoundTripJsonRpcClient> RoundTripJsonRpcClient::Create(smithy::ClientConfig config) {
  ...
  if (!config.endpoint.empty()) {
    ...
    if (transport == nullptr) {
      ...
      if (endpoint->tls()) {
        return smithy::Error::Validation("RoundTripJsonRpcClient: https endpoints need a TLS-capable transport (set config.http_client, e.g. smithy::http::BeastHttpClient::FromEndpoint)");
      }
...
smithy::Outcome<PutSinkRpcOutput> RoundTripJsonRpcClient::PutSinkRpc(const PutSinkRpcInput& input) const {
  ...
  if (response->status != 200 || is_error) return DeserializePutSinkRpcError(*response);
