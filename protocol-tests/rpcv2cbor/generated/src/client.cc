...
smithy::Outcome<RpcV2ProtocolClient> RpcV2ProtocolClient::Create(smithy::ClientConfig config) {
  ...
  if (!config.endpoint.empty()) {
    ...
    if (transport == nullptr) {
      ...
      if (endpoint->tls()) {
        return smithy::Error::Validation("RpcV2ProtocolClient: https endpoints need a TLS-capable transport (set config.http_client, e.g. smithy::http::BeastHttpClient::FromEndpoint)");
      }
