...
TrustedProxies::TrustedProxies(const std::vector<std::string>& cidrs) {
  ...
  for (const std::string& cidr : cidrs) {
    ...
    if (!base.has_value()) {
      smithy::internal::Fatal("TrustedProxies: unparseable address in \"" + cidr + "\"");
    }
    ...
    if (slash != std::string::npos) {
      ...
      if (!AllDigits(digits) || digits.size() > 3) {
        smithy::internal::Fatal("TrustedProxies: malformed prefix in \"" + cidr + "\"");
      }
      prefix_bits = 0;
      ...
      if (prefix_bits < 0 || prefix_bits > max_bits) {
        smithy::internal::Fatal("TrustedProxies: prefix out of range in \"" + cidr + "\"");
      }
    }
